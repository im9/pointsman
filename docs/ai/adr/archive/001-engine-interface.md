...
**Created**: 2026-04-30
**Implemented**: 2026-05-02 (m4l reference impl, 10/10 vectors pass)

This ADR defines the interface contracts and shared test vectors for both
generators. It flipped to "Implemented" once the m4l reference
implementation (`m4l/engine/turing.ts` + `quantizer.ts`) passed every
vector case under `pnpm -r test`. Host integration — verifying the engine
loads correctly under `[node.script]` in a real device — is owned by
[ADR 002](002-m4l-architecture.md), not by this ADR. Other targets
(e.g. `vst/`) re-verify the same vectors under their own ADRs.
...
This ADR flips to *Implemented* once both files exist and all vector
cases pass under `pnpm -r test`. Host integration is ADR 002's concern.

## Implementation checklist

- [x] Implement xoshiro128++ + SplitMix64 seeding in `turing.ts`
- [x] Implement `createRegister`, `shiftAndFlip`, `shiftAndForce`,
      `registerToFraction`, `mapToNote`, `tmStep` in `turing.ts`
- [x] Implement `buildScalePitches`, `snapToScale` in `quantizer.ts`; encode
...
- [x] Wire `*.test.ts` files in each engine package to consume vectors and
      assert exact match
- [x] All vector cases passing — status flipped to *Implemented*
      2026-05-02 (engine 10/10 under `pnpm -r test`)
