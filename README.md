...
`vst/` is built end-to-end per [ADR 003][adr3]: engine, APVTS,
...
| Studio One | — | ❌ Out of scope | Studio One's Note FX slot accepts only PreSonus's own bundled MIDI plug-ins; third-party VST3 (and CLAP) are rejected by host policy, unchanged through Studio One 6 / 7 (PreSonus KB / forum). A multi-track MIDI-routing workaround exists (run Pointsman on a separate track and route its MIDI output to an instrument track) but it conflicts with the "MIDI fx, not synth" identity Pointsman is built on — the same instrument-disguise topology rejected for Cubase. Revisit only if PreSonus opens Note FX to third-party plug-ins. |
